...
import (
	...
	"github.com/hypoballad/mempoolhist/v2/mempool"
...
./mempoolhist server`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		defer db.Close()
		c := cron.New()
		conf := mempool.BitcoinConf{
			Rpcconnect: viper.GetString("bitcoin.rpcconnect"),
			Rpcuser:    viper.GetString("bitcoin.rpcuser"),
			Rpcpasswd:  viper.GetString("bitcoin.rpcpassword"),
			Rpcport:    viper.GetInt("bitcoin.rpcport"),
		}
		...
		c.AddFunc(viper.GetString("job.download_mementry"), func() {
			if err := job.DownloadMementry(viper.GetBool("root.debug")); err != nil {
				log.Printf("download mementry err: %v", err)
			}
		})
		c.Start()
